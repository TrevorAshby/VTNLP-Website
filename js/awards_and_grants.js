readTextFile("./json/awards_and_grants.json", function(text){
    ...
    for (year in arr) {
        ...
        year_div.className = "award_div2";
        for (pub in data.data[arr[year]]) {
            var award_div = document.createElement("div");
            award_div.className = "award_div";

            var award_title = document.createElement("p");
            award_title.className = "award_title";
            award_title.textContent = data.data[arr[year]][pub].title;
            if (!(data.data[arr[year]][pub].title === "")) {
                award_div.append(award_title);
            }

            var award_awarder = document.createElement("p");
            award_awarder.className = "award_awarder";
            award_awarder.textContent = ", " + data.data[arr[year]][pub].awarder;
            if (!(data.data[arr[year]][pub].awarder === "")) {
                award_div.append(award_awarder);
            }

            year_div.append(award_div);
